...
const CheckQueueZZ = React.lazy(
  () => import("./pages/check-queue/CheckQueueZZ")
);
...
function App() {
  ...
  useEffect(() => {
    const fetchTitle = async () => {
      const fetchedTitle = await getTitle();
    ...
    <HelmetProvider>
      <Router>
        <Helmet>
          ...
          <link
            rel="icon"
            href="https://dadashop-backend.vercel.app/api/v1/image/thumbnail"
          />
        </Helmet>
        <NavbarWrapper />
        <ScrollToTop />
        ...
        <Suspense
          fallback={
            ...
            >
              <svg
                aria-hidden="true"
                className="inline w-[100px] h-[100px] text-gray-200 animate-spin dark:text-white fill-blue-600"
                viewBox="0 0 100 101"
                fill="none"
        ...
        >
          <Routes>
            ...
            <Route
              path="/change-image"
              element={<ProtectedRoute component={ChangeImages} />}
            />
            <Route
              path="/shop-setting"
              element={<ProtectedRoute component={ShopSettings} />}
            />
            <Route
              path="/update-preset"
              element={<ProtectedRoute component={UpdatePreset} />}
            />
            <Route
              path="/create-preset"
